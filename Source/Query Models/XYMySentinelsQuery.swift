...
public extension XYMySentinelsQuery {
    ...
    func addSentinel(
        ...
        self.mutateAndAlterCache(for: mutation, query: MyDevicesQuery(), with: { data, _ in
            let update = MyDevicesQuery.Data.MySentinel.Item(
                id: id, name: name, photoUrl: photoUrl, publicKey: publicKey,
    ...
    func updateSentinel(
        ...
        self.mutateAndAlterCache(for: mutation, query: MyDevicesQuery(), with: { data, response in
            ...
            let gps = LastSentinelGps(
                latitude: response?.data?.updateSentinel?.lastGps?.latitude,
                longitude: response?.data?.updateSentinel?.lastGps?.longitude)
            let update = MyDevicesQuery.Data.MySentinel.Item(
                id: id, name: name, photoUrl: photoUrl, publicKey: publicKey,
                uuid: uuid, major: major, minor: minor, lastGps: gps)
